(function() {
  ...
  window.console.log = function(msg) {
    originalAlert('console.log called! check leftover logs')
  };
  ...
  test('map with 12 address markers (geocoding overflow)', function() {
    ...
    map.gMap({
      ...
      onComplete: function() {
        ...
        ok(alertCalled, 'Alert called on overflowed markers');
        window.alert = alert;
  ...
  test('changeSettings - move center', function() {
    ...
    map.gMap({
      ...
      onComplete: function() {
        ...
        window.setTimeout(function() {
          var center = map.data('gmap').gmap.getCenter();
          ok(Math.abs(center.lat() - 50.20917) < 0.01, 'center latitude correct');
          ok(Math.abs(center.lng() - 19.75435) < 0.01, 'center longitude correct');
          start();
        }, 1000);
      }
    });
      ...
      onComplete: function() {
        ...
        window.setTimeout(function() {
          ...
          window.setTimeout(function() {
            ...
            window.setTimeout(function() {
              var center = map.data('gmap').gmap.getCenter();
              ok(Math.abs(center.lat() - (50.083 + 50.502343) / 2) < 0.01, 'center latitude correct');
              ok(Math.abs(center.lng() - (19.917 + 19.75435) / 2) < 0.01, 'center longitude correct');
              start();
            },1000);
          },1000);
        },1000);
